      ...
      body: ListView(
        ...
        children: [
            ...
            data: (reminder) => _MenuCard(
              children: [
                  ...
                  trailing: Switch(
                    ...
                    onChanged: (value) async {
                      ...
                      try {
                        if (value) {
                          ...
                          if (!granted) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'Permission de notification refusée. Active-la dans les paramètres.',
                                ),
                              ),
                            );
                            return;
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(value ? 'Rappel activé ✓' : 'Rappel désactivé'),
                          ),
                        );
                      } catch (e) {
                        if (!context.mounted) return;
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Erreur notification : $e')),
                        );
                      } finally {
                ...
                _ActionTile(
                  ...
                  subtitle: 'Reçois une notification immédiatement',
                  trailing: const Icon(Icons.chevron_right_rounded),
                  onTap: () async {
                    final notif = ref.read(notificationServiceProvider);
                    final granted = await notif.requestPermissions();
          ...
          const Center(
            child: Column(
              children: [
                Text('Sandokti', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 15)),
                SizedBox(height: 4),
                Text('Version 1.0.0', style: TextStyle(color: Colors.black54, fontSize: 12)),
              ],
            ),
          ),
        ],
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: const LinearGradient(
            ...
            colors: [Color(0xFF0B7A43), Color(0xFF0F9F58)],
          ),
          boxShadow: [
            BoxShadow(
        ...
        child: Column(
          children: [
            ...
            Text(fullName, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)),
            const SizedBox(height: 4),
            Text(email, textAlign: TextAlign.center, style: TextStyle(color: Colors.white.withOpacity(0.88), fontSize: 13)),
            const SizedBox(height: 18),
            Row(
              children: [
                Expanded(child: _ProfileActionButton(icon: Icons.person_outline_rounded, label: 'Profil', onTap: onTap)),
                const SizedBox(width: 10),
                Expanded(child: _ProfileActionButton(icon: Icons.notifications_none_rounded, label: 'Rappels', onTap: () {})),
              ],
            ),
          ],
        ),
